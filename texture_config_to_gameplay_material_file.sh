...
grep material_ "${texture_config_file}"|grep color|sed 's/material_//;s/_color//;s/#/0x/'|gawk '{r=rshift(strtonum($3),16)/255; g=and(rshift(strtonum($3),16),0xff)/255; b=and(strtonum($3),0xff)/255; print "material " $1 "_0 : colored\n{\n\t u_diffuseColor = " r ", " g ", " b ", 1\n}\n"}' >> "${gameplay_material_file}"
